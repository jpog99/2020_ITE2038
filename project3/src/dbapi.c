...
int init_db(int buf_num){
    ...
    buf_pool.mru_frame = -1;
    buf_pool.lru_frame = -1;
...
int db_find(int tid, int64_t key, char* ret_val){
    ...
    for(i=0 ; i<MAX_TABLE_COUNT ; i++){
    	if(tid_list[i].table_id == tid){
    	   return find(tid, key, ret_val);break;
    	}
    }
...
int close_table(int table_id){
    ...
    while(last_frame->lru_next !=-1){
    	int next_frame_idx = last_frame->lru_next;
    	if(last_frame->table_id == table_id)
    	    buf_remove_page(buf_pool.lru_frame);
    	buf_pool.lru_frame = next_frame_idx;
    	last_frame = buf_pool.frames + buf_pool.lru_frame;
    }
...
int shutdown_db(){
    ...
    //	if(tid_list[i].table_id <= 0) continue;
    //	close_table(tid_list[i].table_id);
    //}
